...
const UploadReel = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      uploadFormData.append('video', videoFile); // backend expects 'video' field
      uploadFormData.append('foodName', formData.foodName);
      uploadFormData.append('caption', formData.caption);
      uploadFormData.append('cuisineType', formData.cuisineType);
      
      if (formData.price) {
        uploadFormData.append('price', formData.price);
      }

      // Add order links if provided
      if (formData.zomatoUrl || formData.swiggyUrl) {
        const orderLinks = {};
        if (formData.zomatoUrl) orderLinks.zomato = formData.zomatoUrl;
        if (formData.swiggyUrl) orderLinks.swiggy = formData.swiggyUrl;
        uploadFormData.append('orderLinks', JSON.stringify(orderLinks));
      }

      toast.loading('Uploading reel...', { id: 'upload' });
      
      // Use the reelsAPI.uploadReel method with progress callback
      const response = await reelsAPI.uploadReel(
        uploadFormData,
              ...
              <div className="bg-white rounded-3xl border border-gray-200 p-6 shadow-sm">
                ...
                {!videoPreview ? (
                  <div
                    ...
                    className={`relative border-2 border-dashed border-gray-300 rounded-2xl p-12 text-center hover:border-violet-500 hover:bg-violet-50/50 transition-all ${
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 bg-gray-50 border ${
                        errors.foodName ? 'border-red-300' : 'border-gray-200'
