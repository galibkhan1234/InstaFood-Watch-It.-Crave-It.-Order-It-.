...
const Home = () => {
  ...
  // 🔥 FETCH FEED — stable reference via useCallback
  const fetchFeed = useCallback(async (pageNum, refresh = false) => {
    const token = localStorage.getItem("token");
    if (!token || token === "null" || token === "undefined") {
      console.warn("No valid token found, skipping fetchFeed");
      return;
    }
  ...
  // ✅ INFINITE SCROLL — uses ref so page is never stale
  useEffect(() => {
    if (inView && hasMore && !isLoading) {
      pageRef.current += 1;
      fetchFeed(pageRef.current);
    }
  }, [inView, hasMore, isLoading, fetchFeed]);
